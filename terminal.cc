...
int my_chsh();                             // Смена директории
void newProcess(vector<ProcessInfo> &processes, int &cnt_process,
                vector<char *> &args,
                string &str_input); // создание нового процесса
int printPath();                    // Печать текущего пути
void deleteTerminatedProcess(
    int process_number,
    int status); // Удаление информации о завершенном процессе
...
int main() {
  ...
  sa.sa_flags = SA_RESTART /*| SA_NOCLDSTOP*/; // Флаги: не перезапускать, не
                                               // ловить остановленные процессы
  ...
  while (true) {
    ...
    vector<char *> args =
        splitStr(str_input); // Разделяем строку на отдельные аргументы и
                             // записываем в вектор
...
void newProcess(vector<ProcessInfo> &processes, int &cnt_process,
                vector<char *> &args, string &str_input) {
...
vector<char *> splitStr(string &str_input) {
  ...
  str_input = " " + str_input +
              " "; // Добавляем пробел в начало и конец для корректой работы
  if (str_input == "  ")
    return vector<char *>(); // Возвращаем пустой вектор

  // Удаляем лишние пробелы
  i = 0;
  while (i + 1 < str_input.size() && str_input[i + 1] == ' ')
    str_input.erase(i + 1, 1);
  ...
  for (size_t i = 0; i < str_input.size(); i++) {
    if (str_input[i] == '\\')
      str_input.erase(
          i,
          1); // Удаляем символ '\', i теперь указывает на экранируемый символ,
              // на следующей итерации i++ перепрыгнет этот экранируемый символ
    else if (str_input[i] == '"') {
      flag_double_quot = !flag_double_quot;
      str_input.erase(i, 1);
      i -= 1; // После удаления символы смещаются на 1 назад, необходимо
              // вернуться на 1 символ назад, чтобы попасть на следующий символ
    } else if ((str_input[i] == ' ') && (!flag_double_quot)) {
      if (i - ind_space > 1) {
        args.push_back(new char[i - (ind_space + 1) + 1]);
        strcpy(args.back(),
               str_input.substr(ind_space + 1, i - (ind_space + 1)).c_str());
      }
      ind_space = i;
    }
  }
...
int changeDir(vector<char *> args, string home_dir) {
  ...
  } else if (chdir(home_dir.c_str()) !=
             0) // Переходим в домашнюю директорию после команды "cd"
    perror("chdir() error");
...
string getProcessStatus(int status) {
  if (WIFEXITED(status)) {
    ...
    if (!exitCode)
      ...
      return "exit code " +
             to_string(exitCode); // Процесс завершился с ненулевым кодом
  } else if (WIFSIGNALED(status)) {
    int signal = WTERMSIG(status);
    return "terminated by signal " +
           to_string(signal); // Процесс был завершён сигналом
  } else if (WIFSTOPPED(status)) {
    int signal = WSTOPSIG(status);
    return "stopped by signal " +
           to_string(signal); // Процесс был приостановлен сигналом
  } else
...
void my_ps(vector<ProcessInfo> processes) {
  ...
  for (auto process : processes)
    printf("%-5d\t%-10d\t%s\n", process.process_number, process.pid,
...
void deleteTerminatedProcess(int pid, int status) {
  for (size_t i = 0; i < processes.size(); i++) {
    if (processes[i].pid == pid) {
      printf("\n%s[%d] %d %s%s\n", PINK, processes[i].process_number,
             processes[i].pid, getProcessStatus(status).c_str(),
...
int my_exit(vector<ProcessInfo> &processes, vector<char *> &args) {
  ...
  for (size_t i = 0; i < processes.size(); i++) {
    if (processes[i].process_number == number) { // Сравниваем номер
      ...
      printf("%s[%d] %d %s%s\n", PINK, processes[i].process_number,
             processes[i].pid, getProcessStatus(status).c_str(), RESET);
      processes.erase(processes.begin() +
                      i); // Удаляем структуру процесса из вектора
      break;
    }
  }
  if (!flag)
    printf("exit: a process with this number does not exist\n");
...
int printPath() {
  ...
  string cur_dir(cwd); // string cur_dir = filesystem::current_path().string();
  if (!cur_dir.empty()) {
    printf("%s%s@%s%s:", GREEN, cur_user.c_str(), host_name.c_str(), RESET);
    if (cur_dir.find(home_dir) == 0)
      printf("%s~%s%s$ ", CYAN,
             cur_dir.substr(home_dir.size(), cur_dir.size()).c_str(), RESET);
    else
...
int my_chsh() {
  string command = "chsh -s /bin/bash";
